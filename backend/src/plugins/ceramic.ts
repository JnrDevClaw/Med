...
declare module 'fastify' {
  interface FastifyInstance {
    ceramic: {
      ...
      createComposeClient: (opts?: any) => ComposeClient;
      authenticateDID: (seedHex: string) => Promise<DID>;
    };
  }
}
...
export default fp(async function (fastify, opts) {
  ...
  const authenticateDID = async (seedHex: string): Promise<DID> => {
  ...
  const createComposeClient = (opts: any = {}) => new ComposeClient({ ceramic, ...opts });
